...
> **최신 제작 우선순위 (2026-09-22):** 최종 이미지 제작·편집은 `.claude/blog-image-project-instructions-v2.md`가 우선한다. 실제 원본 사진을 GPT 편집의 입력으로 쓰고 장면을 재생성하지 않는다. 아래의 소스·카드 정보 규칙은 유지하되, 원본 간판·워터마크 제거, 사진 블러, 글자 그림자, 별도 도구로 최종 이미지 조판 등 V2와 충돌하는 항목은 적용하지 않는다. 원본 보존 편집이 두 번 실패하면 원본 사진을 그대로 본문에 넣고 정보는 네이버 본문 텍스트로 전달한다.
