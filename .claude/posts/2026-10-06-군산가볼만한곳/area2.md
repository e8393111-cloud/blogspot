# 영역2 — 군산 가볼만한곳 (2026-10-06)

> ## ★★먼저 읽을 것 — 이 글은 **영역2를 완성할 수 없다**
> `photos/CREDITS.md`의 조사 결과가 이렇다.
> - **글자를 얹을 수 있는(Type1) 장소가 동국사(129509) 한 곳뿐**이다.
> - 초원사진관·경암동 철길마을·군산항은 전부 **Type3(변경금지)** — 글자·크롭·워터마크 전부 불가, 원본만.
> - ★**이 글의 주인공인 군산근대역사박물관이 TourAPI에 아예 없다.** 터미널·맛집도 관광지 미등재라 없다.
>
> CLAUDE.md는 "실배경 사진 소스가 없는 카드는 미완성(가짜 생성 배경으로 채우지 않는다)"이라고 못 박았다.
> 그래서 **9장 중 실사진 배경이 있는 건 2장뿐**이고, 나머지는 **카드 문구·프롬프트를 다 준비해 두되
> 배경은 '미확보'로 표시**한다. 운영자가 사진을 넣는 순간 그대로 생성하면 된다.
>
> **세트 색**: 딥네이비 `#27384A` — 군산 근대건축의 벽돌·목조 대비에서 고른 차분한 색.
> 근대역사 소재라 화려한 색을 쓰지 않는다. ★`NOT orange, NOT amber` — 이 글감은 따뜻한 톤이 아니다.
> 핵심 숫자·라벨·구분선에만 15% 이하.

**사용법 3줄 (배경 사진이 있는 카드 공통)**
① 아래 '실배경 사진 소스' URL을 열어 원본을 저장한다
② 이미지 생성 GPT에 그 사진을 업로드한다
③ 아래 영문 프롬프트를 그대로 넣어 **글자만 얹는다**(장면을 새로 그리게 하지 않는다)

---

## 이미지 1. 썸네일 · 1:1 — ✅배경 있음
삽입 위치 : 대표(썸네일)
실배경 사진 소스 : **동국사(군산)** contentid 129509 · `cpyrhtDivCd Type1`
　https://tong.visitkorea.or.kr/cms/resource/15/4088715_image2_1.jpg
　★`detailImage2`로 개별 사진 유형을 한 번 더 확인할 것(대표가 Type1이어도 개별은 다를 수 있다)
카드 텍스트 :
　제목 : 군산 가볼만한곳
　태그라인 : 기차보다 버스가 빨라요
　(이모지 없음 — 썸네일문구의 🚌는 본문 텍스트에만)
영문 프롬프트 :
```
...
삽입 위치 : area1.md 73행
실배경 사진 소스 : **미확보.** 버스터미널은 관광지 등재 대상이 아니라 TourAPI에 없다(정상).
　찾는 방법 — ①운영자가 현장에서 직접 촬영(가장 확실) ②군산시 문화관광 홈페이지 보도자료 사진의 재사용 조건 확인
카드 텍스트 :
　제목 : 터미널에서 근대역사거리까지
　부제 : 기차 말고 버스로 오세요
...
삽입 위치 : area1.md 86행
실배경 사진 소스 : **초원사진관** contentid 2605927 · **`Type3`(제1유형+변경금지)**
　http://tong.visitkorea.or.kr/cms/resource/53/3039553_image2_1.jpg
★처리 : **글자·크롭·워터마크를 일절 얹지 않는다.** 원본 파일을 그대로 본문에 넣고,
　이미지 아래 블로그 캡션으로 `사진 출처: 한국관광공사 포토코리아(공공누리 제1유형)`만 적는다. 생성 단계 없음.

## 이미지 4. 반나절 코스 요약(타임라인) · 4:5 — ⛔배경 미확보 → 사진 없이 간다
삽입 위치 : area1.md 108행
실배경 사진 소스 : **미확보.** 지점별 실사진(터미널·박물관)이 없다.
★`photo-tool.md`의 원칙대로 **작은 사진 없이 번호+글자만으로** 만든다. AI 생성 컷을 쓰지 않는다.
카드 텍스트 :
　제목 : 군산 반나절 코스
　부제 : 터미널 도착 10시 기준
...
삽입 위치 : area1.md 130행
실배경 사진 소스 : **미확보.** ★**이 글의 주인공인데 TourAPI에 등재 자체가 없다.**
...
삽입 위치 : area1.md 183행
실배경 사진 소스 : **미확보**(박물관 사진 자체가 없다).
카드 텍스트 :
　제목 : 휠체어로도 다닐 수 있나요
　부제 : 근대역사박물관은 무장애 시설을 갖췄어요
　✅ 엘리베이터 있음 · 휠체어 5대 무료 대여(안내데스크)
　✅ 주 출입구 무턱 + 자동문 · 경사로 있음
　✅ 장애인화장실 1층·3층 · 장애인 주차 8면
　✅ 전동보장구 충전기(야외 공중화장실 옆)

## 이미지 9. 메인 CTA · 16:9 — ✅배경 있음(동국사 재사용)
...
## 점검 결과
1. 카드만 보고 다음 행동 되나 — 이미지2에 버스번호·하차정류장·플랜B 있음 **YES**
2. 코스요약(이미지4)이 본문 타임라인과 100% 일치 **YES**
3. 세트색 `#27384A` 하나로 통일, 앰버 미사용 **YES**
4. 패널 0개(글자는 사진 밝은 여백에 직접) **YES**
